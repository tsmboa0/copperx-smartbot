...
const sendMessage = tool(
    async({ctx, message})=>{
        ...
        try{
            if(!ctx.update?.message?.chat?.id) return;

            await bot.api.sendMessage(ctx.update?.message?.chat?.id, message, {parse_mode:"Markdown"})
    ...
    },
    {
        ...
        schema: z.object({
            ...
            ctx:z.custom<Context>().describe("This is the Context to be passed to the telegram API")
        })
    }
)
...
const Login = tool(
    async({ctx})=>{
        if(!ctx.update?.message?.chat?.id) return;
        const newCtx = getContext(ctx.update.message?.from.id);
